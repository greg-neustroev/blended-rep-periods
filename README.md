...
2. **Cluster** the time series into `n_rep_periods` representative periods
   (`k_means`, `k_medoids`, `hierarchical`, `chronological`, or greedy
   convex-/convex-with-null-/conical-hull selection).
...
4. **Formulate** the JuMP investment-and-operations LP over the representative
   periods and **solve** it. The inter-period storage chain reconstructs the
   seasonal state-of-charge trajectory with exact-inflow injection.
5. **Evaluate** (optional) the true cost by fixing the reduced model's
   investment (`investment_regret`) or storage (`storage_regret`, graded
   day-exact) decisions in a full-horizon model and re-solving.
...
| `clustering_type` | how representatives are selected | `k_means`, `k_medoids`, `hierarchical`, `chronological`, `convex_hull`, `convex_hull_with_null`, `conical_hull` |
| `weight_type`     | how periods blend representatives | `dirac`, `convex`, `conical`, `conical_bounded` |
| `tol`             | projected-gradient weight-fit tolerance ε (optional) | float (default `0.01`) |
| `normalization`   | clustering-feature scaling (optional) | `economic`, `unscaled`, `minmax` |
| `evaluation_type` | full-horizon regret evaluation | `none`, `investment_regret`, `storage_regret` |

Selection is chosen independently of the weight class. Storage `storage_regret` is
graded day-exact and the inter-period storage chain always uses exact-inflow injection
(the operational weights reconstruct the seasonal trajectory; there is no separate
chain-weight matrix). See the files under [`inputs/`](inputs/) for complete examples and
...
Five real datasets across three independent sources (European TYNDP, US Sienna,
US RTS-GMLC), ranging from a 5-bus illustrative system to a 73-bus case at
5-minute resolution (105 408 timesteps), all held-out comparison systems. Method
development (sensitivity + ablation + the data-property knob study) is done on a
separate **synthetic** system, `synth/hydro`, generated by
[`scripts/gen_synth_sensitivity.py`](scripts/gen_synth_sensitivity.py) — a small
binding-storage network with orthogonal generator knobs, so the method is never
tuned on the test data. See [`inputs/README.md`](inputs/README.md) for the real
datasets' sizes, provenance, and per-dataset modelling notes.
