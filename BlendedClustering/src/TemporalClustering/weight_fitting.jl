...
selects the (non-negative) weight class; both are fit by per-period projected gradient
descent on the same centred-increment target and differ ONLY in the projection:

  - `:convex`  — convex (simplex) fit, partition-of-unity rows;
  - `:conical` — conical (nonnegative-orthant) fit.

Neither class carries a column-sum-zero closure gauge (their column sums are positive);
annual balance is earned by dispatch through the cyclic storage constraint, exactly as the
operational weights do. Non-negativity (interpolation) is the property that keeps the
reconstructed seasonal trajectory well-behaved — the proven chain-weight mechanism.
...
increment-reconstruction error (the hull-containment error — small when the base increments
are in-hull, large in the sign-changing seasonal regime).
"""
function fit_chain_weights(
  ...
  # Centre each asset row to net-zero over the year (cyclic closure): 1ᵀG = 0. Both
  ...
  # Per-base-period projected gradient descent; the two (non-negative) classes share the
  # SAME PGD and target and differ ONLY in the projection's feasible set (simplex for
  # convex, the nonnegative orthant for conical). Annual balance is earned by dispatch
  # through the cyclic constraint, exactly as the operational weights.
  projection =
    weight_type ≡ :convex  ? project_onto_simplex :
    weight_type ≡ :conical ? project_onto_nonnegative_orthant :
    throw(ArgumentError("Unsupported chain weight_type $(weight_type); expected :convex or :conical"))
  ...
  # Relative reconstruction residual ‖G_R W^chᵀ − G‖_F / ‖G‖_F: the hull-containment
  # error for these bounded classes (small when the base increments are in-hull, large
  # in the sign-changing seasonal regime).
